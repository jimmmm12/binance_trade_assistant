...
from trade_assistant.risk_engine import PlanRiskReview, evaluate_plan_risk, suggest_leverage
...
def run_scan(
    ...
    *,
    ...
    for item in markets:
        ...
        all_longs.extend(_score_many(longs, mode))
        all_shorts.extend(_score_many(shorts, mode))
...
def _score_many(signals: list[Signal], mode: str) -> list[ScoredSignal]:
    return [score_signal(signal, mode=mode) for signal in signals]
...
def auto_plan_prices(signal: Signal | ScoredSignal, mode: str) -> AutoPlanPrices:
    ...
    adaptive = adapt_parameters(base, mode)
    if mode == "swing":
    ...
    risk_note = (
        ...
        f"自适应风险 {adaptive.risk_pct:.2f}%，建议杠杆≤{adaptive.suggested_leverage:.1f}x"
    )
...
def detect_positions(
    ...
    if signal is None:
        advice = PositionAdvice(action="wait", summary="请先选择扫描信号，再判断开仓/加仓/减仓", warnings=[])
    else:
...
def evaluate_plan_from_form(
    ...
    plan = create_plan_from_form(symbol, market, side, entry, stop, target, equity, risk_pct, leverage)
    base_signal = signal.signal if isinstance(signal, ScoredSignal) else signal
    settings = load_settings()
    return plan, evaluate_plan_risk(
        plan,
        base_signal,
        position,
        mode,
        min_live_score=int(settings.get("min_live_score", 70)),
    )
