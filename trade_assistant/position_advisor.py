...
def advise_position(
    ...
    if signal.breakdown.action_level == "block_live":
        return PositionAdvice(action="block", summary="评分系统判定不交易", warnings=warnings)
    ...
    if not has_position:
        if signal.breakdown.action_level in {"tradeable", "small_trade"}:
    ...
    add_allowed = signal.breakdown.add_allowed or (
        signal.breakdown.grade == "未分级"
        and signal.score >= ADD_THRESHOLD
        and signal.breakdown.action_level in {"tradeable", "small_trade"}
    )
    if add_allowed and pnl >= 0:
        return PositionAdvice(action="add", summary="已有同向仓位且信号仍强，可考虑小幅加仓", warnings=warnings)
    reduce_recommended = signal.breakdown.reduce_recommended or (
        signal.breakdown.grade == "未分级" and signal.score < REDUCE_THRESHOLD
    )
    if reduce_recommended or pnl < 0:
